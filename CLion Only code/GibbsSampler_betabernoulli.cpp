...
/*struct MatrixCompare {
    bool operator()(const MatrixXd& lhs, const MatrixXd& rhs) const {
        return lhs.isApprox(rhs); // Compare matrices for approximate equality
    };
}*/
...
std::list<Eigen::MatrixXd> GibbsSampler_betabernoulli( double alpha, double theta, double sigma_x,double sigma_a,  int n_tilde,  int n,  Eigen::MatrixXd A, Eigen::MatrixXd X, unsigned n_iter, unsigned initial_iters){
  ...
  */
   // Rcout << "Dimensioni di A: " << std::endl;
    // Initialization of Z and m:


    Eigen::MatrixXd Z = Eigen::MatrixXd::Zero(n, n_tilde);
...
//Alla fine di ogni iterazione calcolo la quantità log[P(X|Z)]
        ...
        std::cout << "eq_21_log_denominator + eq_21_log_exponential = " << eq_21_log_denominator << " + " << eq_21_log_exponential << " = " << eq_21_log << endl;
...
*/
        ...
        cout << "eq_12_log = eq_12_log_fraction + eq_12_log_product = " << eq_12_log_fraction << " + " << eq_12_log_product << " = " << eq_12_log << endl;
