---
title: "Call_GibbsSamplerBB_updateA"
...
data <- data.frame(
  iteration = 1:(n_iter + initial_iters),
  K = result$K_vector,
  logPXZ = result$logPXZ_vector,
  sigmaA = result$sigmaA_vector,
  sigmaX = result$sigmaX_vector
)

# Creazione dei grafici individuali
plot_k <- ggplot(data, aes(x = iteration, y = K)) +
  geom_line() +
  labs(title = 'Number of latent features', y = 'K') +
  geom_vline(xintercept = initial_iters, linetype = 'dashed', color = 'red') +
  ylim(6, 9)  # Aggiunta ylim qui

plot_logPXZ <- ggplot(data, aes(x = iteration, y = logPXZ)) +
  geom_line() +
  labs(title = 'log P(X, Z)', y = 'log P(X, Z)') +
  geom_vline(xintercept = initial_iters, linetype = 'dashed', color = 'red') +
  ylim(-200, -100)  # Aggiunta ylim qui

plot_sigmaA <- ggplot(data, aes(x = iteration, y = sigmaA)) +
  geom_line() +
  labs(title = 'sigma A', y = 'sigma A') +
  geom_vline(xintercept = initial_iters, linetype = 'dashed', color = 'red') +
  ylim(0, 30)  # Aggiunta ylim qui

plot_sigmaX <- ggplot(data, aes(x = iteration, y = sigmaX)) +
  geom_line() +
  labs(title = 'sigma X', y = 'sigma X') +
  geom_vline(xintercept = initial_iters, linetype = 'dashed', color = 'red') +
  ylim(0, 10)  # Aggiunta ylim qui

combined_plots <- plot_grid(plot_k, plot_logPXZ, plot_sigmaA, plot_sigmaX, nrow = 2)
